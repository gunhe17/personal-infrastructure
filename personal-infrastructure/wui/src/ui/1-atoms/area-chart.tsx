...
export type Series = { name: string; points: number[]; tone?: keyof typeof COLOR; color?: string; fill?: boolean; dash?: boolean };
...
export function AreaChart({ series, max, height = 160, format = (v) => String(v), formatMark, xLabels, legend = true, annotate = false, nowLabel, nowClass, nowColor, nowWidth, axisWidth = 36, limit, className }: { series: Series[]; max?: number; height?: number; format?: (v: number) => string; formatMark?: (v: number) => string; xLabels?: string[]; legend?: boolean; annotate?: boolean; nowLabel?: React.ReactNode; nowClass?: string; nowColor?: string; nowWidth?: number; axisWidth?: number; limit?: { value: number; label?: React.ReactNode }; className?: string }) {
  const W = 1000, top = Math.max(max ?? 0, limit?.value ?? 0, ...series.flatMap((s) => s.points)) || 1;
  ...
  const chart = (
    <div className="relative min-w-0 flex-1" style={{ height }}>
      <svg viewBox={`0 0 ${W} ${height}`} preserveAspectRatio="none" className="absolute inset-0 size-full overflow-visible" aria-hidden="true">
        ...
        {limit && <line x1={0} x2={W} y1={y(limit.value)} y2={y(limit.value)} stroke="var(--mute)" strokeOpacity=".6" strokeWidth="1" strokeDasharray="4 4" vectorEffect="non-scaling-stroke" />}
        {annotate && P.length > 1 && <>
        ...
        {series.map((s) => { const c = colorOf(s); return (
          <g key={s.name}>
            ...
            <path d={path(s.points)} fill="none" stroke={c} strokeWidth="2" strokeLinejoin="round" strokeDasharray={s.dash ? "5 4" : undefined} vectorEffect="non-scaling-stroke" />
      ...
      {limit?.label !== undefined && <span className="pointer-events-none absolute end-0 -translate-y-full pb-1 font-mono text-[11px] leading-4 tabular-nums text-mute" style={{ top: y(limit.value) }}>{limit.label}</span>}
    </div>
  );
