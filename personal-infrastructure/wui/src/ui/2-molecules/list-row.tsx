...
export function ListRow({ lead, title, sub, value, end, onClick, className }: { lead?: React.ReactNode; title: React.ReactNode; sub?: React.ReactNode; value?: React.ReactNode; end?: React.ReactNode; onClick?: () => void; className?: string }) {
  const body = <>{lead}<div className="min-w-0 flex-1"><p className="truncate text-body-lg font-medium text-text">{title}</p>{sub && <p className="truncate text-body text-mute">{sub}</p>}</div>{value !== undefined && <span className="text-body-lg tabular-nums text-text">{value}</span>}{end && <span className="flex shrink-0 items-center gap-2">{end}</span>}</>;
