...
- 2026-09-09 크기·간격 정리: 척도 밖 값 기계 점검(정당한 예외 8곳 확인, DotMatrix 14px 만 16 으로 교정) · DeviceList 의 첫·끝 행 여백을 지워 카드 머리 아래 40 → 24(다른 카드와 일치) · 확정 컴포넌트의 치수(소제목 줄 28 · 작은 세그먼트 24 · 그래프 64/32 · 막대 8 · 점 6 · 눈금 36 · 값 88)와 카드 안 목록 리듬 둘을 design.md 척도 표에 정식 등록.
- 2026-09-09 크기 통일(사용자 지정 — 리소스3 = `DeviceList` 기준): 행 이름·카드 안 소제목 17 → **15/500**, 부속 줄 15 → **13**, 행 안 값 17 → 15. ListRow·Accordion·ActionPanel·Fieldset·Popover·Callout·Tile(이니셜)·RecentDeploys 적용. 목록 타일 48 → sm 36, 행 안 배지 36 → sm 32. `body-lg`(17)은 상단 바 이름만 남는다.
- 2026-09-09 캔버스 크기 불일치 수정(사용자 지적): 리소스 보드만 `wide` 로 1120→640(57%) 축소돼 다른 컴포넌트보다 작게 보였다. 트랙을 640→960 으로 넓히고 `wide` 축소 렌더를 제거해 템플릿(frame 50%)을 뺀 모든 보드가 100%. 캔버스 여백 p-16→p-12 로 1280 뷰포트에 맞춤.
- 2026-09-09 리소스 크기를 standard 로: 캔버스 100% 렌더에서 재보니 세그먼트 트랙만 48 로 문서(40)·버튼(40)과 어긋났다 — SEG 칸 40 → 32(px-5 → px-4)로 낮춰 Tabs·FilterTabs·ToggleGroup 전부 40. 나머지 컨트롤(버튼 40 · 입력 44 · 배지 36 · 칩 36 · 타일 sm 36 · 작은 세그 24 · 카드 제목 22/28)은 규격과 일치 확인.
- 2026-09-09 캔버스 분류 개선(사용자 요청 — 다른 라이브러리 참고): 층 안 묶음을 역할 어휘로 통일(동작·입력·표시·피드백·내비게이션·오버레이·표면), 층마다 같은 순서, 묶음 안은 알파벳 순. 왼쪽 라벨 열을 목차로(묶음 + 개수 + 앵커), 묶음 머리에 hairline. 컴포넌트 누락 없음(id 대조 검증).
- 2026-09-09 Layout Lab 개설(`/ui/#layout`, `pages/LayoutLab.tsx`, 캔버스 도구 줄 layers 아이콘). 관리자 콘솔 레이아웃 다섯을 같은 본문·같은 1280×800 프레임(50%)으로 비교: L1 상단 바 + 가운데 폭(Vercel·Railway, 지금 AppShell) · L2 왼쪽 사이드바 240(Grafana·Portainer·Proxmox) · L3 아이콘 레일 64 + 구역 내비 200(Linear·Sentry·Cloudflare) · L4 상단 바 + 대상 탭(GitHub) · L5 레일 + 목록–상세(Fly.io·k8s). 내비 항목은 ui-feature-map 의 12 구역. 발견: DeviceList 는 3열이라 600 아래에서 접힌다 — 반 폭 카드에 넣지 말 것.
- 2026-09-09 **확정**: 셸은 2열 사이드바 — [프로젝트 목록][그 프로젝트의 항목][본문]. Layout Lab 을 이 구조 안의 디자인 다섯으로 재구성: S1 아이콘 레일 64 + 항목 200 · S2 이름 열 220 + 항목 200 · S3 두 열을 같은 card 바탕 + hairline · S4 레일 + 2열 위 프로젝트 머리(이름·상태·도메인·Deploy) · S5 1열을 Projects / System 으로 나눔. 사이드바가 420 을 가져가면 본문에 DeviceList(600 필요)를 2열로 못 둔다 — 본문은 한 열로.
- 2026-09-09 **확정**: 셸 1열은 **아이콘 레일 64**(S1). Layout Lab 을 1열 확장 방식 넷으로 재구성: X1 고정 확장 토글(64 ↔ 220, 본문이 밀림, 상태 기억 · Linear·VS Code) · X2 hover 오버레이(레일 유지, 이름 패널이 본문 위로 enter-side) · X3 스위처 팝오버(확장 없음, 검색 있는 목록 enter-drop) · X4 한 열 병합(240 안에 프로젝트 + 고른 것의 항목, 열이 3 → 2). 레일에서 실패 상태는 타일 모서리 점.
- 2026-09-09 **확정·적용**: 셸을 Layout Lab X4(한 열 병합)로 결정하고 키트에 반영 — 새 유기체 `SideNav`(프로젝트 + 고른 것의 항목 + System), `AppShell` 을 [SideNav 240][본문] 으로 재작성(상단 바 제거). 템플릿 셋(Analytics·DetailScreen·SettingsScreen)과 Lab 둘을 새 셸로 옮김. 본문 폭 1040.
- 2026-09-09 실제 페이지 시작: `#login` → `#home`. `pages/Login.tsx`(SignInCard 하나, 셸 밖) · `pages/Home.tsx`(SideNav + 빈 상태 + Next steps 3단계 CLI) · `lib/session.ts`(sessionStorage 토큰 하나 — 서버 붙일 때 이 파일만). 토큰 없이 `#home` 이면 로그인으로. SignInCard 문구 영어로. AppShell 높이 `min-h-dvh`(본문이 짧아도 사이드바가 화면 끝까지). e2e W8 추가 — 로그인 → 홈 → 빈 상태 · 사이드바 확인(총 8).
